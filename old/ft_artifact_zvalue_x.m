...
% TOEGEVOEGD DOOR JOHANNES/joram OM OP DE abs(min(Z)+cutoff) TE THRESHOLDEN
if strcmp(cfg.artfctdef.zvalue.flexible, 'yes')
    if thresholdsum 
        opt.threshold = median([zsum{:}]) + abs(min([zsum{:}])-median([zsum{:}])) + cfg.artfctdef.zvalue.cutoff;
    else 
        opt.threshold = median([zmax{:}]) + abs(min([zmax{:}])-median([zmax{:}])) + cfg.artfctdef.zvalue.cutoff;
    end
else
...
if strcmp(cfg.artfctdef.zvalue.interactive, 'yes')
  ...
  redraw_cb(h);
  saveas(h,[cfg.datafile(1:end-4) '.png'], 'png');
